...
- **Alerting**: `prometheus/rules/bothive.yml` ships 20 alert rules (API down/high error rate/slow p95, all-workers-down, workers down, queue backlog/high-wait/stuck failed jobs, unhealthy bots/proxies, script failure spikes, worker heap growth/reconnect thrashing/sandbox worker leaks, crypto high error rate/no fills/volume spike, plus SLO burn-rate/latency pages). Prometheus evaluates them automatically; the bundled Alertmanager (`alertmanager.yml`) currently uses a null receiver — edit it to add a webhook/email and start getting notified.
