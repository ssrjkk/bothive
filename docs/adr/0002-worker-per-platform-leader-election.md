...
The system orchestrates bots across five platforms (Telegram, Twitch, YouTube, Twitter, and crypto/Binance). Events arrive from platform SDK callbacks and custom scripts; bot actions (say, timeout, ban, react) are executed by workers. Two requirements drove the topology:
...
- Run **one BullMQ worker process per platform** (`workers-telegram`, `workers-twitch`, `workers-youtube`, `workers-twitter`, and opt-in `workers-crypto`), each consuming its own queue and sharing the same workers image (`node --import ./dist/tracing-preload.js ./dist/index.js --platform <name>`).
