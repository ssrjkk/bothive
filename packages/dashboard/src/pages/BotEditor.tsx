...
function BotEditor() {
  ...
  useEffect(() => {
    api
      .get<PatternSpec[]>('/scripts/patterns')
      .then(setPatterns)
      .catch(() => undefined);
  }, []);
  ...
  const selectPattern = (pid: string) => {
    ...
    if (p)
      for (const spec of p.params) if (spec.default !== undefined) init[spec.key] = spec.default;
    setGenValues(init);
  };
  ...
  const fetchMemory = () => {
    ...
    api
      .get<MemoryEntry[]>(`/bots/${id}/memory`)
      .then(setMemory)
      .catch(() => setMemory([]))
  ...
  return (
    <div>
        ...
        extra={
          <Space wrap>
            ...
            {bot.status !== 'running' ? (
              ...
              <Button icon={<PauseCircleOutlined />} onClick={() => botAction('stop')}>
                Stop
              </Button>
            )}
          </Space>
        ...
        items={[
          {
            ...
            children: (
              <Card className="bh-card" variant="borderless">
                ...
                <Input.TextArea
                  rows={8}
                  defaultValue={JSON.stringify(bot.config, null, 2)}
                  onBlur={(e) => updateConfig(e.target.value)}
                ...
                <Table
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      dataIndex: 'enabled',
                      key: 'enabled',
                      render: (e: boolean, record: { id: string }) => (
                        <Switch checked={e} onChange={(v) => toggleScript(record.id, v)} />
                      ),
                    },
                    {
                      title: 'Actions',
          ...
          },
          {
            ...
            children: (
              <Card
                className="bh-card"
                variant="borderless"
                title={<span style={{ fontWeight: 700 }}>Generate script from pattern</span>}
              >
                <Space direction="vertical" style={{ width: '100%' }} size={16}>
                  <Space wrap>
                    ...
                    {selectedPattern && (
                      <Space size={4}>
                        {selectedPattern.platforms.map((p) => (
                        ...
                        <Space key={spec.key} style={{ width: '100%' }} wrap>
                          ...
                          {(spec.type === 'string' || spec.type === 'text') && (
                            <Input.TextArea
                              ...
                              value={genValues[spec.key] as string | undefined}
                              onChange={(e) =>
                                setGenValues((prev) => ({ ...prev, [spec.key]: e.target.value }))
                              }
                            />
                          )}
                        </Space>
          ...
          },
          {
            ...
            children: (
              <Card
                className="bh-card"
                variant="borderless"
                title={<span style={{ fontWeight: 700 }}>Recent logs</span>}
              >
                <Table
                  dataSource={bot.logs}
                  columns={[
            ...
            children: (
              ...
              >
                ...
                >
                  ...
                  <Form.Item
                    name="payload"
                    label={`Payload (JSON) — e.g. ${actionExamples[actionType]}`}
                  >
                    <Input.TextArea
                      rows={6}
                      placeholder={actionExamples[actionType]}
                      style={{ fontFamily: 'monospace', fontSize: 12.5 }}
                    />
                  </Form.Item>
                  <Button
          ...
          },
          {
            ...
            children: (
              <Card
                ...
                extra={
                  <Space>
                    <Button size="small" icon={<ReloadOutlined />} onClick={fetchMemory}>
                      Refresh
                    </Button>
                    ...
                    },
                    {
                      ...
                      render: (_: unknown, record: MemoryEntry) => (
                        <Popconfirm
                          ...
                          onConfirm={() => deleteMemoryKey(record.key)}
                        >
                          <Button
                            size="small"
                            danger
                            icon={<DeleteOutlined />}
                            aria-label={`Delete memory key ${record.key}`}
                          />
                        </Popconfirm>
                      ),
                    },
      ...
      <Modal
        title={
          <span>
            <CodeOutlined style={{ color: token.colorPrimary, marginRight: 8 }} />
            Add Script
          </span>
        }
      ...
      >
        <Form form={scriptForm} layout="vertical" onFinish={createScript}>
          ...
          <Form.Item name="config" label="Config (JSON)" rules={[{ required: true }]}>
            <Input.TextArea
              rows={8}
              placeholder='{"filters":[{"type":"regex","value":"!hello"}],"actions":[{"type":"reply","payload":{"text":"Hi there!"}}]}'
              style={{ fontFamily: 'monospace', fontSize: 12.5 }}
            />
          </Form.Item>
        </Form>
