  ...
  const columns = [
    {
      ...
      render: (name: string, record: Bot) => (
        ...
        >
          <span
            className="bh-dot"
            style={{ background: platformHex(record.platform), marginRight: 8, verticalAlign: 1 }}
          />
          {name}
        </a>
      ),
        ...
        <Space>
          {record.status === 'running' ? (
            <Button
              ...
              onClick={() => botAction(record.id, 'stop')}
            >
              Stop
            </Button>
          ) : (
            <Button
              ...
              loading={actionLoading === record.id}
              disabled={record.status === 'connecting' || record.status === 'reconnecting'}
              onClick={() => botAction(record.id, 'start')}
            >
              Start
            </Button>
          )}
          <Button
            size="small"
            loading={actionLoading === record.id}
            onClick={() => botAction(record.id, 'restart')}
          >
            Restart
          </Button>
  ...
  return (
    <div>
        ...
        title={
          <Space wrap>
            ...
            <Select
              ...
              options={['running', 'idle', 'paused', 'error', 'connecting', 'reconnecting'].map(
                (s) => ({ value: s, label: s }),
              )}
            />
          </Space>
      ...
      >
        <Table
          ...
          pagination={{
            ...
            showTotal: (t) => `${t} bot${t === 1 ? '' : 's'}`,
          }}
          sticky
          locale={{
            emptyText: (
              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No bots match your filters">
                <Button type="primary" icon={<PlusOutlined />} onClick={() => setModalOpen(true)}>
      ...
      <Modal
        title={
          <span>
            <RobotOutlined style={{ color: token.colorPrimary }} /> Create Bot
          </span>
        }
        open={modalOpen}
