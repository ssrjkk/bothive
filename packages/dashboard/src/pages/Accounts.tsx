...
function Accounts() {
  ...
  const openEdit = (account: Account) => {
    setEditing(account);
    // Reset first so stale credential values typed for another account are not
    // carried into this edit.
    form.resetFields();
  ...
  const columns = [
    ...
    { title: 'Platform', dataIndex: 'platform', key: 'platform', render: (p: string) => <Tag>{p}</Tag> },
    {
      ...
      render: (_: unknown, record: Account) => {
        ...
        return names.length === 0 ? <Tag>none</Tag> : names.map((n) => <Tag key={n} color="blue">{n}</Tag>);
      },
    },
    {
      title: 'Bots', key: 'bots',
      render: (_: unknown, record: Account) => {
        ...
        if (linked.length === 0) return <span>{record._count.bots}</span>;
        return (
          <Space direction="vertical" size={2}>
            <span>{linked.length} bot{linked.length === 1 ? '' : 's'}</span>
            <Space size={4} wrap>
              {linked.map((b) => (
                <Tag key={b.id} color={statusColors[b.status] ?? 'default'}>{b.name} · {b.status}</Tag>
              ))}
            </Space>
          </Space>
        );
  ...
  return (
    <div>
      ...
      <Table dataSource={accounts} columns={columns} rowKey="id" loading={loading} pagination={{ pageSize: 20 }} />
