...
- Ship alert rules (`prometheus/rules/bothive.yml`, 20 rules: API down/high error rate/slow p95, all-workers-down, workers down, queue backlog/high-wait/stuck failed jobs, unhealthy bots/proxies, script failure spikes, worker heap growth/reconnect thrashing/sandbox worker leaks, crypto high error rate/no fills/volume spike, plus SLO burn-rate/latency pages) evaluated by Prometheus.
