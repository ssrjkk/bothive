...
function Scripts() {
  ...
  const columns = [
    ...
    { title: 'Bot', key: 'bot', render: (_: unknown, record: ScriptRow) => (record.bot ? <Space size={6}>{record.bot.name}<PlatformTag platform={record.bot.platform} /></Space> : <Tag style={{ borderRadius: 999 }}>—</Tag>) },
    ...
    {
      title: 'Actions', key: 'actions', width: 268,
  ...
  return (
    <div>
      <PageHeader
        ...
        extra={<Button icon={<ReloadOutlined />} onClick={fetchScripts}>Refresh</Button>}
      />
      <Card
        className="bh-card"
        variant="borderless"
        title={
          <Space wrap>
            <Input.Search allowClear placeholder="Search scripts by name" style={{ width: 240 }} onChange={(e) => setSearch(e.target.value)} />
            <Select allowClear placeholder="Bot" style={{ width: 200 }} onChange={setBotFilter} options={bots.map((b) => ({ value: b.id, label: `${b.name} (${b.platform})` }))} />
            <Select allowClear placeholder="Trigger" style={{ width: 140 }} onChange={setTriggerFilter} options={triggers.map((t) => ({ value: t, label: t }))} />
            <Space>
              ...
              <Typography.Text type="secondary">{selectedKeys.length > 0 ? `${selectedKeys.length} selected` : ''}</Typography.Text>
            </Space>
          </Space>
        }
      >
      ...
      <Modal
        title={<span><CodeOutlined style={{ color: token.colorPrimary }} /> Edit script — {editing?.name ?? ''}</span>}
        open={!!editing}
      ...
      >
        <Form form={editForm} layout="vertical">
          ...
          <Form.Item label="Config (JSON)">
            <Input.TextArea rows={10} value={configText} onChange={(e) => setConfigText(e.target.value)} style={{ fontFamily: 'monospace', fontSize: 12.5 }} />
          </Form.Item>
        </Form>
      </Modal>
    </div>
