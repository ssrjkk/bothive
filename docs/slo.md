...
| Job latency     | `bothive_queue_jobs_total{state="waiting"}` / `bothive_worker_queue_depth` | how long control jobs wait for a consumer                                                          |
...
- Alerting is burn-rate based, multi-window: a short, very hot window (`14.4×` for `2m`) catches a full outage immediately; a cooler but persistent window (`6×` over `6h` for `15m`) catches slow degradation. This avoids both "alerting on every 5xx" and "noticing a real SLO breach after a week".
